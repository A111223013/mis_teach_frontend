...
export const Tooltip = mvc.View.extend({
    ...
    render: function(options) {
        ...
        let element = null;
        if (options.target) {
            [element] = $(options.target).closest(this.options.target);
        }
    ...
    position: function(targetBBox) {
        ...
        this.$('.tooltip-arrow, .tooltip-arrow-mask')
            .attr('style', null) // Reset style of previous tooltip
    ...
    getViewportViewBBox: function() {
        ...
        const { settings } = this;
        const { selector: viewportSelector, padding: viewportPadding = 0 } = settings.viewport;

        let el;
        if (viewportSelector) {
            [el] = $(settings.currentTarget).closest(viewportSelector);
        } else {
            el = document.documentElement;
        }

        const viewportBBox = util.getElementBBox(el);
