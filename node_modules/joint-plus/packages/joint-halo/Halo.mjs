import { V, g, util, dia, mvc } from '@joint/core';
...
ElementHalo.prototype.makeLoopLink = function(link) {
    const { loopLinkPreferredSide, loopLinkWidth: linkWidth, paper } = this.options;
    const { width, height } = paper.getComputedSize();
    const paperRect = new g.Rect({ x: 0, y: 0, width, height });
    const bbox = paper.paperToLocalRect(this.options.cellView.getBBox());
    let p1, p2;

    const sides = util.uniq([loopLinkPreferredSide, 'top', 'bottom', 'left', 'right']);
    const sideFound = sides.find(function(side) {
        ...
        switch (side) {
            ...
            case 'top':
                centre = new g.Point(bbox.x + bbox.width / 2, bbox.y - linkWidth);
                dx = linkWidth / 2;
                break;

            case 'bottom':
                centre = new g.Point(bbox.x + bbox.width / 2, bbox.y + bbox.height + linkWidth);
                dx = linkWidth / 2;
                break;

            case 'left':
                centre = new g.Point(bbox.x - linkWidth, bbox.y + bbox.height / 2);
                dy = linkWidth / 2;
                break;

            case 'right':
                centre = new g.Point(bbox.x + bbox.width + linkWidth, bbox.y + bbox.height / 2);
                dy = linkWidth / 2;
                break;
        }
