import { V, util, mvc } from '@joint/core';

const $ = mvc.$;

export const toSVG = (paper, callback, opt) => {
    ...
    const exportSVG = V(svg).clone();
    let exportNode = exportSVG.node;
    const exportViewport = exportSVG.findOne('.' + util.addClassNamePrefix('layers'));
    ...
    if (!opt.grid) {
        // Remove the grid.
        exportSVG.findOne('.' + util.addClassNamePrefix('grid-layer')).remove();
    }
...
function copyExternalStyles(fromNode, toNode) {
    ...
    fromElements.forEach((el, idx) => {
        ...
        util.forIn(computedStyle, (property) => {
...
export const openAsSVG = (paper, opt) => {
