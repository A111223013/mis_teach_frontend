...
export const SwimlaneTransform = dia.ToolView.extend({
    ...
    updateLaneHeight: function(laneId, diff, handleSide, shapeBBox, shapeAngle) {
        ...
        const neighbours = this.getNeighbourLanesIds(model, laneId, handleSide);
        const currentLane = this.getSublanesAndMostEmbeddedLaneId(model, laneId, handleSide);
    ...
    getSublanesAndMostEmbeddedLaneId: function(model, laneId, handleSide) {
        const sublaneIds = this.getIdsOfClosestSublanesToTheSide(model, laneId, handleSide);
    ...
    getNeighbourLanesIds: function(model, originLaneId, handleSide) {
        const oppositeHandleSide = this.getOppositeHandleSide(handleSide);
        const { foundNeighbourLaneId, laneIdsPathToNeighbour } = this.findNeighbourLane(model, originLaneId, handleSide);
        const sublaneIds = this.getIdsOfClosestSublanesToTheSide(model, foundNeighbourLaneId, oppositeHandleSide);
    ...
    findNeighbourLane: function(model, originLaneId, side) {
        const { metrics } = model;
        ...
        while (tempLaneId) {
            const laneMetrics = model.getLaneMetrics(tempLaneId);
            ...
            if (isNeighbourOutsideCurrentParent) {
                ...
                foundNeighbourLaneId = this.getSublaneIdOfParent(metrics.lanes, laneParentId, neighbourIndex);
                break;
            }
        }
    ...
    getSublaneIdOfParent: function(lanes, parentLaneId, sublaneIndex) {
        const allLaneIds = Object.keys(lanes);

        return allLaneIds.find(laneId => {
            const laneMetrics = lanes[laneId];
    ...
    getIdsOfClosestSublanesToTheSide: function(model, parentLaneId, side) {
        const pathToSublane = [];
        let tempSublaneId = parentLaneId;

        while (tempSublaneId) {
            const sublaneMetrics = model.getLaneMetrics(tempSublaneId);
            ...
            const sublaneExists = model.getLaneMetrics(sublaneId) !== null;
    ...
    getMaxConstraintsToEmbeds: function(model, laneId, handleSide) {
        ...
        const topNeighbourLaneId = this.findNeighbourLane(model, laneId, HandleSides.Top).foundNeighbourLaneId;
        const bottomNeighbourLaneId = this.findNeighbourLane(model, laneId, HandleSides.Bottom).foundNeighbourLaneId;
