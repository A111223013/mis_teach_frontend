...
var layoutDefaults = {
    ...
    options: function() {
        ...
        return {
            // makes margin from both sides
            columnWidth: (this.options.width - 2 * marginX - columnGap * (columns - 1)) / columns,
            columns,
            rowHeight: 80,
            rowGap: 10,
            columnGap,
...
export const Stencil = mvc.View.extend({
    ...
    initializeLayout: function() {
        ...
        if (layout) {
            ...
            if (util.isFunction(layout)) {
                ...
                this.options.layout = util.isObject(layout) ? layout : layoutDefaults.options.call(this);
            }
        }
    },
    ...
    createDraggingPaper: function() {
        ...
        const graphDrag = this._graphDrag = paperDragOptions.model ||
            paperOptions.model || new dia.Graph({}, { cellNamespace: this.defaultCellNamespace });

        const dragPaperConfig = util.assign({
        ...
        const paperDrag = new dia.Paper(dragPaperConfig);
    ...
    renderToggleAll: function() {
        ...
        return [
            $('<div/>').addClass('groups-toggle')
                ...
                .append($('<button/>').addClass('btn btn-expand').text('+'))
                .append($('<button/>').addClass('btn btn-collapse').text('-'))
        ];
    },
    ...
    render: function() {
        ...
        if (hasGroups) {
            ...
            sortedGroups.forEach(function(name) {
                ...
                const groupPaperConfig = util.assign({
                    cellViewNamespace: this.defaultCellViewNamespace
                }, groupPaperOptions, {
                    el: groupPaperEl,
                    model: groupGraph,
                ...
                const groupPaper = new dia.Paper(groupPaperConfig);
    ...
    getGroupPaperOptions: function(groupId) {
        ...
        if (groupId) {
            const groupPaperOptions = util.result(this.getGroup(groupId), 'paperOptions');
            optionsCascade.push(groupPaperOptions);
        }
        return util.assign(...optionsCascade);
    },

    createGroupGraph: function(groupId, paperOptions) {
        let graph;
        if (!paperOptions.model) {
            graph = new dia.Graph({}, { cellNamespace: this.defaultCellNamespace });
        } else {
    ...
    cancelDrag: function({ dropAnimation } = this.options) {
        ...
        if (dropAnimation) {
            ...
            const { duration = 150, easing = 'ease-in-out' } = dropAnimation;
    ...
    filter: function(keyword, cellAttributesMap = this.options.search) {
        ...
        var match = Object.keys(this.papers).reduce((wasMatch, group) => {
            ...
            var matchedCells = paper.model.getCells().filter((cell) => {
                ...
                var cellMatch = false;
                if (!cellAttributesMap) {
                    // search is not defined, so we assume that none of the cells matches
                } else if (util.isFunction(cellAttributesMap)) {
