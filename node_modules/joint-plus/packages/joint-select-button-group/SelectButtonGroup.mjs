...
export const SelectButtonGroup = mvc.View.extend({
    ...
    renderOptionContent: function(option, isSelected) {
        ...
        var $option = $('<div/>').addClass('select-button-group-button').html(option.content);
        if (option.icon || (isSelected && option.iconSelected)) {
            var $icon = $('<img/>')
                .addClass('select-button-group-button-icon')
                .attr('src', isSelected && option.iconSelected ? option.iconSelected : option.icon);
