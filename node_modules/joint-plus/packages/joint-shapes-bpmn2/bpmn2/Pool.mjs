...
export const Pool = dia.Element.define('bpmn2.Pool', {
    ...
    getLaneMetrics(laneId) {
        const { lanes, lanesNameCache } = this.metrics;

        const laneKeyInMetrics = (laneId in lanesNameCache ? lanesNameCache[laneId] : laneId);

        return lanes[laneKeyInMetrics] || null;
    },
    ...
    getLanePath: function(laneId) {
        let laneMetrics = this.getLaneMetrics(laneId);
    ...
    attributes: {
        ...
        'label-position': {
            position: function(_, refBBox, node, attrs) {
                ...
                // rotate margins
                const { left: top, top: left, right: bottom, bottom: right } = util.normalizeSides(attrs['label-margin']);
        ...
        'milestone-label-position': {
            position: function(_, refBBox, node, attrs) {
                ...
                const { left, top, right, bottom } = util.normalizeSides(attrs['label-margin']);
                const halfHeight = height / 2;
                const halfWidth = width / 2;
                const alignment = attrs['label-alignment'];
        ...
        'text-wrap': {
            set: function(opt, refBBox, node, attrs) {
                ...
                dia.attributes['text-wrap'].set.call(this, opt, refBBox, node, attrs);
            }
        },
