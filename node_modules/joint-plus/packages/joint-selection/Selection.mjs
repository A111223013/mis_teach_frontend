...
export const Selection = mvc.View.extend({
    ...
    adjustSelection: function(evt) {
        ...
        const { action, scrollWhileDragging, interactionPrevented = false } = data;
            ...
            case 'translating': {
                ...
                if (dx || dy) {
                    ...
                    if (!this.boxesUpdated) {
                        ...
                        // 2. If the paper is in async mode, we always update the selection boxes.

                        // 3. Correctly update selection box when there is one element with allowCellInteraction
    ...
    createSelectionWrapper: function() {
        ...
        var $selectionWrapper = $('<div/>')
            ...
            .data('selectionLength', this.model.length);
    ...
    getCellBBox(cell) {
        const { paper, useModelGeometry } = this.options;
        const view = paper.findViewByModel(cell);
        if (!view) return null;
        return view.getBBox({ useModelGeometry });
    },
    ...
    getWrapperBBox() {
        ...
        return g.Rect.fromRectUnion(...cellBBoxes);
    },
    ...
    updateSelectionWrapper: function() {
        const { collection, $selectionWrapper } = this;
        let x, y, width, height;
        const wrapperBBox = this.getWrapperBBox();
        if (wrapperBBox) {
            ({ x, y, width, height } = wrapperBBox);
        } else {
    ...
    _updateSelectionBoxes: function() {
        ...
        for (var i = 0, n = children.length; i < n; i++) {
            ...
            var removedId = $(element).remove().data('model');
