...
*/
import { util, dia, mvc } from '@joint/core';
import { SelectBox } from '../joint-select-box/SelectBox.mjs';
import { SelectButtonGroup } from '../joint-select-button-group/SelectButtonGroup.mjs';
import { ColorPalette } from '../joint-color-palette/ColorPalette.mjs';
import { RadioGroup } from '../joint-radio-group/RadioGroup.mjs';
...
export const Inspector = mvc.View.extend({
    ...
    renderOwnLabel: function(options, path) {
        ...
        return $('<label/>').html(options.label || path).toArray();
    },
        ...
        switch (opt.type) {
            ...
            case 'select':
                ...
                util.toArray(items).forEach(function(item) {
                    ...
                    var $option = $('<option/>').text(item.content).val(item.value);
            ...
            case 'toggle':
                ...
                $button = $('<span><i/></span>');
                $input = $('<input/>').attr('type', 'checkbox').prop('checked', !!opt.value);
            ...
            case 'color':
                ...
                $input = $('<input/>').attr('type', 'color').val(opt.value);
            ...
            case 'text':
                ...
                $input = $('<input/>').attr('type', 'text').val(opt.value);
    ...
    renderTemplate: function(el, options, path, opt) {
        ...
        if ((options.type === 'list') && options.item) {
            ...
            util.toArray(value).forEach(function(itemValue, index) {
                const listItemEl = this.renderListItem({ index, options });
            ...
            attributesArray.forEach(function(attribute) {
                const objectPropertyEl = this.renderObjectProperty({ property: attribute.path });
    ...
    getFieldValue: function(attributeEl, type) {
        ...
        switch (type) {
            ...
            case 'select':
                return attributeEl.multiple
                    ? Array.from(attributeEl.selectedOptions).map((option) => option.value)
                    : attributeEl.value;
            case 'content-editable':
    ...
    addListItem: function(evt) {
        ...
        var $attribute = parentEl.matches('[data-attribute]') ? $(parentEl) : $(); // parent element of $collection
        ...
        var addedItemEl = this.renderListItem({ index, options });
